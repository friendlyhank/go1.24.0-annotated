...
Go 编译器简介
cmd/compile 包含构成 Go 编译器的主要包。编译器逻辑上可分为四个阶段

1.解析阶段
包：cmd/compile/internal/syntax（词法分析、语法解析、生成语法树）
作用：
词法分析：将源代码拆分为有意义的标记（token），如变量名、运算符等。
语法解析：将标记序列转换为抽象语法树（AST），精确表示源代码结构。
错误报告：语法树包含位置信息，用于定位错误和生成调试信息

2.类型检查（Type checking）
包：cmd/compile/internal/types2（类型检查）
作用：
类型验证：检查变量、函数调用等是否符合类型规则。
兼容性：types2 是 go/types 的移植版本，使用 syntax 包的 AST 替代 go/ast。

3.IR 构建（Noding）
包：
cmd/compile/internal/types（编译器类型系统）
cmd/compile/internal/ir（编译器 AST）
cmd/compile/internal/noder（生成编译器 AST）

作用：
转换表示：将类型检查后的 AST 转换为编译器内部使用的 IR（中间表示）。
统一 IR：通过序列化类型检查结果构建 IR，支持包导入/导出和内联。
历史遗留：编译器中间端沿用 C 时代的 AST 和类型表示。

4.中间端优化（Middle end）
   包：
   cmd/compile/internal/inline（函数内联）
   cmd/compile/internal/devirtualize（接口方法调用去虚拟化）
   cmd/compile/internal/escape（逃逸分析）
   作用：
   优化 IR：执行多种中间表示优化，如死代码消除、去虚拟化、函数内联、逃逸分析。
   早期优化：死代码消除整合到 Unified IR 写入阶段。
典型优化：
移除未使用的函数参数。
将 switch 语句转换为二进制搜索表或跳转表。

5.代码遍历（Walk）
包：cmd/compile/internal/walk（计算求值顺序、语法糖展开）
作用：
分解复杂语句：将复杂语句拆解为简单语句，引入临时变量并保持求值顺序。
语法糖展开：将高级 Go 语法转换为更基础的表示，例如：
switch 转换为二进制搜索表。
map 和 channel 操作替换为运行时调用。

6.通用 SSA 阶段（Generic SSA）
作用：
SSA 转换：将 IR 转换为静态单赋值形式（SSA），便于后续优化和机器码生成。
内联函数：替换特定函数为高度优化的代码（如 append 的扩容逻辑）。
降低复杂度：将高级语法（如 copy）转换为内存操作。
通用优化：移除冗余 nil 检查、合并分支、常量折叠等。
关键概念：
SSA：一种中间表示形式，每个变量仅赋值一次，便于优化。
机器无关优化：适用于所有架构（如 GOARCH=amd64 和 GOARCH=arm64）。

7.机器码生成（Generating machine code）
包：
cmd/compile/internal/ssa（SSA 降低和架构相关优化）
cmd/internal/obj（机器码生成）
作用：
降低（Lowering）：将通用 SSA 转换为目标架构的特定表示（如 AMD64 支持内存操作数）。
最终优化：执行寄存器分配、栈帧布局、指针活跃分析等。
生成目标文件：将 SSA 转换为 obj.Prog 指令，由汇编器生成最终机器码。
关键步骤：
栈帧布局：为局部变量分配栈偏移。
指针活跃分析：确定垃圾回收时哪些栈上指针有效。

7a. 导出数据（Export）
作用：
跨包编译：生成导出数据文件（export data），供下游编译单元使用。
内容：包含导出声明的类型信息、内联函数的 IR、泛型函数的实例化信息、逃逸分析结果。
格式演进：
Unified 格式：对象图的序列化表示，支持按需懒加载。
Indexed 格式：旧格式，用于工具链（如 gopls 的工作区数据库）。
应用场景：
编译依赖：包 Q 直接导入 P 时，仅需读取 P 的导出文件。
性能权衡：Deep 导出（全量信息）适合分布式构建，但会增大文件；Shallow 导出（仅直接信息）适合低开销场景（如 gopls）。

8. 开发技巧（Tips）
go build -gcflags=-m=2           # 输出内联和逃逸分析信息
go build -gcflags=-W             # 输出类型检查后的 AST
GOSSAFUNC=Foo go build           # 生成 Foo 函数的 ssa.html
